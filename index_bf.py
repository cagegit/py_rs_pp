...
class MyFrame(wx.Frame):
    ...
    def __init__(self, *args, **kw):
        ...
        self.vpn_ctrl = wx.TextCtrl(pnl, 0, value="5", size=(30, -1))
        if self.conf_len > 0:
        ...
        self.output_btn = wx.Button(pnl, label="打开输入结果目录")
        ...
        self.columns = [u'姓', u'名', u'地址', u'邮编', u'州', u'月', u'日', u'年', u'电话', u'州缩写', u'编号', u'邮箱', u'密码', u'ip地址',
                        u'结果']
        self.dataList = []
    ...
    def get_vpn_res(self, event):
        ...
        res = change_proxy_ins.change_ip(country, hwnd=str(hwnd))
        return res
    ...
    def loop_table(self, event):
        ...
        print(country)
        # 写入配置文件
        config['DEFAULT'] = {'BrowserPath': browser_path_text, 'VpnPath': vpn_path_text, 'VpnNumber': vpn_number,
    ...
    def run(self):
        ...
        if web_url and register_call:
            ...
            for idx in range(0, self.parent.table.list.GetItemCount()):
                col_count = self.parent.table.list.GetColumnCount() - 1
                ...
                register_ins = register_call(web_url, account_list)
                register_ins.play()
